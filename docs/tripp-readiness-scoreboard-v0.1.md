...
| Replace Goose for read-only planning/review | 75% | Tripp now passes linear and multi-branch read-only planning threads with inspect, mock retrieval, follow-up inspection, safe shell, blocked shell, and gate review. | Longer-session repeatability and broader session variety. | Prove Tripp stays coherent across repeated 8-10 task read-only sessions without Goose narration. | Goose review + Codex |
...
- The mixed-session or multi-branch acceptance harness fails or becomes flaky.
